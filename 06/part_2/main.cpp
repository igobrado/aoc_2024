...
int main(int argc, char** argv)
{
    ...
    auto input        = parse_lines(lines);
    auto visited_path = get_visited(input.m_obstructions, input.m_guard_position, input.m_dimension);
    if (!visited_path)
    {
        return 0;
    }
    ...
    for (const auto& [x, y] : *visited_path)
    {
        if (input.m_obstructions.contains({x, y})
            || (input.m_guard_position.m_x == x && input.m_guard_position.m_y == y))
        {
            continue;
        }

        input.m_obstructions.insert({x, y});

        if (!get_visited(input.m_obstructions, input.m_guard_position, input.m_dimension))
        {
            ++loops;
        }

        input.m_obstructions.erase({x, y});
    }

    std::puts(std::format("Output: {}", loops).c_str());
}
